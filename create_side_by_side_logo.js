...
const outputPath = path.join(__dirname, 'frontend', 'src', 'assets', 'ethos_side_by_side.png');
...
fs.createReadStream(logoPath)
  ...
  .on('parsed', function() {
    const W = this.width;
    const H = this.height;
    console.log(`Original Logo Image Dimensions: ${W}x${H}`);

    // Crop Symbol (top 0% to 64% of height)
    ...
    // Composition target dimensions
    const targetSymH = 120;
    const targetSymW = Math.floor((symW / symH) * targetSymH);

    const targetTextH = 65;
      ...
      for (let x = 0; x < symW; x++) {
        ...
        if (outX >= 0 && outX < totalW && outY >= 0 && outY < totalH) {
          ...
          if (r > 140 && g < 100 && b < 120) {
            // Luminous Hot Pink / Red Brush Stroke
            out.data[outIdx] = 255;
            out.data[outIdx + 1] = 0;
    ...
    console.log('✓ Created clean side-by-side transparent logo PNG from official logo file!');
  });
