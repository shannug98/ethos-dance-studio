...
const logoPath = 'C:\\Users\\yuvar\\.gemini\\antigravity\\brain\\0356d35a-373b-4472-b305-3a6be009ae27\\.user_uploaded\\media_1787062738101.png';
...
fs.createReadStream(logoPath)
  ...
  .on('parsed', function() {
    ...
    // Crop Symbol (upper ~60% of height)
    const symH = Math.floor(H * 0.60);
    const symW = W;

    // Crop Text Matter (bottom 60% to 92% of height)
    const textStartY = Math.floor(H * 0.60);
    const textH = Math.floor(H * 0.32);
    const textW = W;

    // Target composition heights
    const targetSymH = 100;
    const targetSymW = Math.floor((symW / symH) * targetSymH);

    const targetTextH = 55;
    ...
    for (let y = 0; y < symH; y++) {
      for (let x = 0; x < symW; x++) {
        ...
        if (r > 190 && g > 190 && b > 190) continue; // transparent background
    ...
    console.log('✓ Created perfect side-by-side logo PNG!');
  });
